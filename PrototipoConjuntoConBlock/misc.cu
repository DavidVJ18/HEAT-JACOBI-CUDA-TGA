...
int initialize( algoparam_t *param )
{
	...
	cudaMallocHost((double**)&(param->uvis),sizeof(double)*(param->visres+2) *
								(param->visres+2));
...
{
    if( param->u ) {
	if(PINNED) cudaFreeHost(param->u);
	else free(param->u);
	param->u = 0;
    }

    if( param->uhelp ) {
	if(PINNED) cudaFreeHost(param->uhelp);
	else free(param->uhelp);
	param->uhelp = 0;
    }

    if( param->uvis ) {
	if(PINNED) cudaFreeHost(param->uvis);
	else free(param->uvis);
	param->uvis = 0;
    }
	
	if( param->diffs ) { 
        if(PINNED) cudaFreeHost(param->diffs);
	else free(param->diffs); 
        param->diffs = 0; 
    }
    return 1;
}
...
void print_params( algoparam_t *param ) {
  ...
  fprintf(stdout, "Solver            : %d (%s)\n",
	  param->algorithm,
	  (param->algorithm == 0) ? "Jacobi":
	  (param->algorithm == 1) ? "Gauss-Seidel":"Unknown solver" );
